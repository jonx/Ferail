...
declare_class!(
    ...
    /// dispatch selector for every Ferail-owned command (looked up
    ...
    unsafe impl AppMenuTarget {
        /// Dispatch a Ferail-owned command. The sender's tag is the
...
/// maps to a Ferail-owned [`CommandId`].
...
/// Build a Ferail-owned menu item: looks up the spec, sets the
