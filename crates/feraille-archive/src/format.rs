...
//! off-thread in `feraille-fs-native` when an archive is actually opened.

/// A supported archive format.
///
/// The set is intentionally bounded to the formats Feraille ships codecs for
...
/// land here together with their codec in `feraille-fs-native` and their row
