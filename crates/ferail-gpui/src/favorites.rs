...
//! badge. Persistence is delegated to [`ferail_meta::MetadataDb`];
...
impl Favorites {
    ...
    pub fn shift(&mut self, id: FavoriteId, by: isize, cx: &mut Context<Self>) {
        ...
        // `ferail_core::favorites::reorder_bounds`.
        let indices: Vec<f64> = self.entries.iter().map(|f| f.sort_index).collect();
        let Some((before, after)) = ferail_core::favorites::reorder_bounds(&indices, pos, by)
        else {
            return;
        };
...
/// the real add UI. Active only when `FERAIL_DEV_SEED_FAVORITES=1`
/// and the list is currently empty.
pub fn maybe_seed_dev_favorites(favorites: &mut Favorites, cx: &mut Context<Favorites>) {
    if std::env::var("FERAIL_DEV_SEED_FAVORITES").as_deref() != Ok("1") {
        return;
    }
