use crate::setting_panel::{SettingGroup, SettingPage};
use gpui::{
...
impl RenderOnce for Settings {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        // Fork point: upstream wraps sidebar|content in `h_resizable(...)`
        // with two `resizable_panel()`s. A settings sidebar doesn't need
        // user resizing, and upstream's resizable group re-scaled its
        // panels proportionally one frame late on window resize, making
        // right-anchored controls jump and wrapped text flicker. A plain
        // fixed-width + flex row lets the layout engine absorb the window
