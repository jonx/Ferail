...
//!
//! NB: the optional provider is **mpv** (`ferail-video-mpv`), gated behind
//! the `mpv` cargo feature and selected via the `video_backend` = `"mpv"`
//! setting, with its libmpv path in `mpv_path` (Settings → Plugins).

use std::path::Path;

use ferail_core::video::{VideoBackend, VideoEnhance, VideoStream};
...
/// [`ferail_video_mpv::backend`] resolves the actual dylib from there (and
...
pub fn video_backend(hint: Option<&Path>) -> Box<dyn VideoBackend> {
    ...
    if let Some(path) = hint {
        if let Some(b) = ferail_video_mpv::backend(path) {
            return b;
        }
