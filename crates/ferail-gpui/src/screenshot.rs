...
//!
//! ```sh
//! cargo run --bin ferail-gpui -- --screenshot screenshots/foo.png \
...
EXAMPLES
  ferail-gpui --screenshot home.png --navigate ~/Documents
  ferail-gpui --screenshot multi.png --new-tab ~/Documents --new-tab ~/Downloads --tab 1
  ferail-gpui --screenshot filter.png --navigate . --filter toml --search
  ferail-gpui --screenshot dark.png --theme dark --navigate ~/Documents
"
    );
}
            ...
            let handle = cx
                .open_window(opts, |window, cx| {
                    if let Some(du_root) = disk_usage_root.clone() {
                        ...
                        let fs = std::sync::Arc::new(ferail_fs_native::NativeFs::new());
                        ...
                        let id = ferail_core::favorites::FavoriteId::new();
                        let view = cx.new(|cx| {
