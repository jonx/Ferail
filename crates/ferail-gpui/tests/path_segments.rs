...
use ferail_gpui::shell::path_segments;
...
fn segments_deep_path() {
    let segs = path_segments(Path::new("/Users/alice/Source/Ferail/crates"));
    assert_eq!(segs.len(), 6);
    assert_eq!(segs[0].0, "/");
    assert_eq!(segs[5].0, "crates");
    assert_eq!(
        segs[5].1,
        PathBuf::from("/Users/alice/Source/Ferail/crates")
    );
}
...
fn segments_drive_deep_path() {
    let segs = path_segments(Path::new(r"D:\Source\Ferail\crates"));
    assert_eq!(segs.len(), 4);
    assert_eq!(segs[0].0, r"D:\");
    assert_eq!(segs[3].0, "crates");
    assert_eq!(segs[3].1, PathBuf::from(r"D:\Source\Ferail\crates"));
}
...
fn canonicalize_resolves_symlinks_for_identity() {
    use ferail_gpui::shell::canonicalize_for_identity;
    let base = std::env::temp_dir().join(format!("ferail-canon-test-{}", std::process::id()));
...
fn canonicalize_falls_back_on_missing_path() {
    use ferail_gpui::shell::canonicalize_for_identity;
    let ghost = PathBuf::from("/definitely/not/a/real/path/ferail");
