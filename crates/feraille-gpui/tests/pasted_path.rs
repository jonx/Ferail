...
use feraille_gpui::shell::{parse_pasted_path, resolve_go_to_target};
...
fn resolve_maps_a_file_to_its_folder() {
    ...
    let file = dir.join("feraille-go-to-folder-test.txt");
    std::fs::write(&file, b"x").expect("write temp file");
    let resolved = resolve_go_to_target(file.clone());
    let _ = std::fs::remove_file(&file);
    assert_eq!(
        resolved,
        feraille_gpui::shell::canonicalize_for_identity(dir)
    );
}
...
fn resolve_keeps_a_path_that_does_not_exist() {
    let missing = PathBuf::from("/nonexistent-feraille/deeper/still");
