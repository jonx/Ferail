...
/// callable from this vendored copy. Plain-string descriptions (the only
/// kind Feraille uses) match identically; `TextView` (markdown/html)
/// descriptions don't contribute to search matching (upstream only
/// differs for stateful TextViews).
fn text_content(text: &Text, _cx: &App) -> SharedString {
    match text {
        Text::String(s) => s.clone(),
        Text::TextView(_) => SharedString::default(),
    }
}
        ...
        match self {
            SettingItem::Item {
                ...
                title.to_lowercase().contains(q)
                    || description
                        .as_ref()
                        .map_or(false, |d| text_content(d, cx).to_lowercase().contains(q))
