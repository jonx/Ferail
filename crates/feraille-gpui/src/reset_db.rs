...
fn print_reset_db_usage() {
    ...
    eprintln!();
    eprintln!("Usage:  feraille-gpui --reset-db <scope>");
    eprintln!();
    ...
    eprintln!();
    eprintln!("Examples:");
    eprintln!("  feraille-gpui --reset-db ui          # forget window size + open tabs");
    eprintln!("  feraille-gpui --reset-db caches      # re-sniff magic + re-walk Ant Trail");
    eprintln!("  feraille-gpui --reset-db all         # nuke the DB file outright");
}
