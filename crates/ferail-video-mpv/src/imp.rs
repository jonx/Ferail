...
use ferail_core::video::{ChromaKey, VideoAdjust, VideoBackend, VideoEnhance, VideoStream};
    ...
    LIB.with(|cell| {
        ...
        if slot.is_none() {
            match unsafe { LibMpv::load(hint) } {
                ...
                Err(e) => {
                    eprintln!("[ferail] mpv backend unavailable: {e}");
                    return None;
                }
            }
        }
...
impl VideoBackend for MpvBackend {
    fn open(
        ...
        let log_level = std::env::var("FERAIL_MPV_LOG").unwrap_or_else(|_| "error".into());
