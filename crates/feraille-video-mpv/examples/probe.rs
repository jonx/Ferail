...
//!   cargo build -p feraille-video-mpv --example probe
//!   FERAILLE_MPV_LOG=v lldb --batch -o run -k 'bt' -k 'quit' -- \
//!          target/debug/examples/probe /tmp/rot90.mp4
//!
//! `FERAILLE_MPV_LOG=v` (handled in imp.rs) prints mpv's decoder/VO setup —
//! the lines that name the frame geometry, hwdec, and rotation.

use std::path::Path;
use std::time::{Duration, Instant};

use feraille_core::video::VideoEnhance;
use feraille_video_mpv::backend;

fn main() {
    let mut args = std::env::args().skip(1);
    let Some(path) = args.next() else {
        eprintln!("usage: probe <video> [frames]   (env: MPV_HOME, FERAILLE_MPV_LOG=v)");
        std::process::exit(2);
    };
