...
//! `cargo check --target x86_64-unknown-linux-gnu -p ferail-shell-linux`
...
/// `ferail_shell_mac::ShellInfo` so the Get Info panel reads one type
...
/// Mirrors `ferail_shell_mac::read_shell_info`.
...
/// Spawn a child process fully detached from Ferail: no inherited stdio, and
