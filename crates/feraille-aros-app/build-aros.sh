...
# stale-std guard below) + link-aros.sh (link + deploy C:Feraille).
#
#   crates/feraille-aros-app/build-aros.sh              # debug
...
cargo "+$TOOLCHAIN" build $CARGO_PROFILE_FLAG -p feraille-aros-app \
    --target "$TARGET_JSON" \
