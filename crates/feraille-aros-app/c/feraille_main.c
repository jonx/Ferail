 ...
 * faults that point nowhere near Feraille). Launch scripts set
...
#define FERAILLE_STACK_MIN (8ul << 20)
#define FERAILLE_STACK_RUN (16ul << 20)
...
int main(int argc, char **argv)
{
    ...
    if (current_stack_size() < FERAILLE_STACK_MIN) {
        ...
        if (cli && cli->cli_Module) {
            ...
            if (args)
                while (args[len])
                    len++;
            return RunCommand(cli->cli_Module, FERAILLE_STACK_RUN,
                              len ? args : (CONST_STRPTR)"\n",
                              len ? len : 1);
        }
        PutStr("[FERAILLE] FAIL: stack too small and no CLI to relaunch "
