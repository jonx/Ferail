...
| `fps` | gpui's frame-timing profiler | Frames this gpui window drew over the last sample window, **floored**. Per-window: each window's status bar shows its own figure. |
...
## The fps figure is a tripwire, not a benchmark

gpui only draws invalidated frames, so an idle window draws nothing and the
segment honestly reads `0 fps`. That's a feature: a nonzero fps while the app
should be idle means something is notify-looping. The figure is floored (not
rounded) so the sampler's own 0.5 Hz repaint ticks can never masquerade as
`1 fps`.
...
- The 0.5 Hz sampler notify redraws each window's status bar even when
  ...
  still draws ~1 frame every 2 s (which the floor keeps out of the fps
  figure).
