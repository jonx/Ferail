...
Metal). The spike killed it for our case:
...
transparent windows), so the only remaining CPU work is each window's frame
rasterize+upload. A future opaque-only fast path via `gpui::surface` (YUV, no
