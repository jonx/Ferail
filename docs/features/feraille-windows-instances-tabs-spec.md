# Feraille — Windows, Instances & Tabs Specification
...
- **Process** — one running copy of the `feraille-gpui` executable.
...
The central architectural decision in §1 is: **Feraille is single-process, multi-window.** "Multiple instances" from the user's point of view means multiple windows, all served by one process. This is deliberate and explained below.
...
Feraille runs as **one process** per user session. Launching Feraille again — from the dock, from Finder, from `open -a`, from the CLI — does **not** start a second process. It signals the existing process to open a new window (or activate an existing one).

This is the standard macOS application model (Finder, Safari, Mail all work this way) and it's the right call for Feraille specifically because:

- **The shell is the coordination point.** The architecture doc puts the file watcher, task registry, notifications, preview cache, metadata caches, and `NodeStore` in the shell. These are process-wide resources. Two processes would mean two file watchers fighting over the same FSEvents streams, two SQLite connections to `feraille-meta`, two preview caches doing duplicate work, and two `NodeId ↔ PathBuf` maps that don't agree. `NodeId` identity is only meaningful *within* the process that minted it — cross-process it's noise.
...
- The CLI `feraille` binary for non-GUI utilities (`magic`, `du`) is **separate** and does not participate in the singleton — those are one-shot processes that do their work and exit. Only `feraille-gpui` is the singleton. A CLI verb that wants to *drive the GUI* (e.g. "open this folder in the app") forwards an intent to the primary `feraille-gpui` process if one exists, or launches it.

### 1.3 What "open Feraille" does, by entry point

| Entry point | Behavior |
|---|---|
| Dock icon, no windows open | Open a new window at the default location (last-closed window's folder, or Home). |
| Dock icon, windows already open | Activate the app, bring the frontmost Feraille window forward. Do **not** open a new window. |
| `Cmd+N` in-app | New window. |
| `Cmd+T` in-app | New tab in the current window. |
| Finder "Open With → Feraille" on a folder | If a window is open: new tab (or new window — see §4.3 preference) at that folder, in the frontmost window. If no window: new window at that folder. |
| `open -a Feraille <folder>` | Same as Finder "Open With". |
| Double-click a folder when Feraille is the registered folder handler | Same as Finder "Open With". |
| CLI `feraille-gpui <path>` while primary running | Forward intent to primary: open `<path>` per the §4.3 preference. Launching process exits. |
| CLI `feraille-gpui` while primary running | Forward "new window" intent. Launching process exits. |
...
| **Move tab to another window** | Drag a tab onto another Feraille window's tab strip → the tab moves into that window at the drop index (§3.5). |
...
- Dragging over window B should bring it forward (spring-loading): hovering a background Feraille window during a drag raises it after a short delay so the user can see the drop target. Hovering a background window's tab during a drag activates that tab after a short delay (spring-loaded tabs), so you can drop into any tab of any window.
...
- Feraille publishes standard macOS pasteboard flavors (file URLs, and promised-file flavors where the content may need producing). This is `feraille-shell-mac`'s job — the GPUI layer starts the drag with a `NodeId` payload; shell-mac translates to the platform pasteboard representation by resolving those nodes' paths at the controlled boundary.
- A drag-out that the OS resolves as a **move** (e.g. into a Finder window on the same volume) will result in the source files being moved by the *receiving* side. Feraille's file watcher sees the change and reloads the source view through the shell — Feraille doesn't perform the move itself in this case, it just reacts to the watcher. A drag-out resolved as **copy** likewise: the OS/receiver copies, Feraille's source view is unaffected.
- Promised files: if Feraille ever drags content that isn't a plain on-disk file (a saved-search result set, a synthesized item), it uses the promise mechanism and produces the file when the receiver asks — on a worker, not the UI thread.

### 5.3 Dragging in from Finder or other apps

- Inbound drops arrive as external paths on the pasteboard. They cross the shell-mac boundary, get registered in `NodeStore`, and the operation proceeds as a normal copy/move into the Feraille drop target (selection/DnD spec §3.7).
...
- Modifier semantics for inbound drops follow the OS: dragging from Finder with Option = copy, etc. Feraille reads the drag's proposed operation from the pasteboard/drag info rather than imposing its own.

### 5.4 Distinguishing the three payload types

At any moment a drag in Feraille is exactly one of:
...
A tab drag never leaves the process (you can't tear a Feraille tab into Finder). A favorite drag never leaves the sidebar's Favorites section as a *reorder*, though dropping a node onto Favorites is a node-drag drop, not a favorite drag. A node drag is the only one that crosses the app boundary in either direction.
...
Per the architecture doc, `feraille-meta` stores window/layout state, and persisted state is treated as cache/preference, never as filesystem truth. Session state to persist:
...
- **`feraille-shell-mac`** — the singleton mechanism (Apple Events, `NSApplication` reopen/open-files), the OS pasteboard for cross-app DnD (§5.2–5.3), dock-icon behavior, native window state.
- **`feraille-meta`** — session/window/tab persistence (§6).
- **The shell** (`feraille-gpui::Shell`) — owns the window list, the process-wide task registry/watcher/caches, the intent dispatcher, the cross-window reload fan-out.
- **`feraille-core`** — the command catalogue entries for all window/tab commands (`Cmd+N`, `Cmd+T`, `Cmd+W`, tab cycling, merge windows), so app actions, macOS menu items, and future command-palette share one identity layer.
...
- [ ] Dragging nodes from one Feraille window to another works with `NodeId` identity intact (no path round-trip).
