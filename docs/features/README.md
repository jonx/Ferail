...
| [PRIVATE_MODE.md](PRIVATE_MODE.md) | Shipped with follow-ups | Session-only, process-global capture lock: the prepared UI remains intact while semantic names/paths/metadata are projected, content pixels are hidden, interactions are frozen, and screenshots enable it by default. |
| [SIDEBAR_LAYOUT.md](SIDEBAR_LAYOUT.md) | Implemented; cross-platform visual qualification pending | Persistent section disclosure/order and normal/compact/icon density, with stable IDs for conditional platform sections. |
