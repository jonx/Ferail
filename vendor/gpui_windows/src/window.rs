...
pub(crate) struct WindowsWindowInner {
    ...
    /// Payload waiting for `WM_GPUI_START_EXTERNAL_DRAG`. `SHDoDragDrop`
...
impl PlatformWindow for WindowsWindow {
    ...
    fn start_external_drag(&self, payload: &ExternalDragPayload) -> bool {
        // `SHDoDragDrop` is a synchronous modal loop. This method is called
        // from GPUI's mouse-input dispatch while the application `RefCell` is
        // mutably borrowed; starting OLE here lets its nested message pump
        // re-enter GPUI and panic with "RefCell already borrowed". Posting a
        // private window message starts the session after this callback and
        // GPUI's borrow have both unwound.
        self.0
        ...
        .is_ok();
        if !posted {
            self.0.pending_external_drag.borrow_mut().take();
        }
        posted
    }
...
fn drag_modifiers(key_state: MODIFIERKEYS_FLAGS) -> Modifiers {
    Modifiers {
        control: key_state.contains(MK_CONTROL),
        alt: key_state.0 & MK_ALT != 0,
        shift: key_state.contains(MK_SHIFT),
        platform: false,
        function: false,
    }
}

/// Choose the effect the destination advertises back to OLE. The input
/// `allowed` mask belongs to the source; never claim an operation it omitted.
fn negotiated_drop_effect(key_state: MODIFIERKEYS_FLAGS, allowed: DROPEFFECT) -> DROPEFFECT {
    let modifiers = drag_modifiers(key_state);
    let requested = if (modifiers.control && modifiers.shift) || modifiers.alt {
        DROPEFFECT_LINK
        ...
        // Windows normally moves within a volume and copies across volumes.
        // The GPUI platform layer does not know the hovered Ferail folder, so
        // advertise Move here; the app's Auto transfer resolves the actual
        // volume relationship off-thread before touching either path.
        DROPEFFECT_MOVE
    };
...
impl WindowsDragDropHandler {
    ...
    fn handle_drag_modifiers(&self, key_state: MODIFIERKEYS_FLAGS) {
        let modifiers = drag_modifiers(key_state);
        if self.0.state.last_reported_modifiers.get() == Some(modifiers) {
            return;
        }
        self.0.state.last_reported_modifiers.set(Some(modifiers));
        self.handle_drag_drop(PlatformInput::ModifiersChanged(ModifiersChangedEvent {
            modifiers,
...
impl IDropTarget_Impl for WindowsDragDropHandler_Impl {
    fn DragEnter(
        ...
        unsafe {
            ...
            if idata_obj.QueryGetData(&config as _) == S_OK {
                *pdweffect = negotiated_drop_effect(grfkeystate, allowed_effects);
    ...
    fn DragOver(
        ...
        unsafe {
            *pdweffect = negotiated_drop_effect(grfkeystate, *pdweffect);
            self.0
                .drop_target_helper
...
#[cfg(test)]
mod tests {
    use super::{ClickState, drag_modifiers, negotiated_drop_effect};
    ...
    #[test]
    fn ole_drag_modifiers_and_effect_follow_windows_conventions() {
        let all = DROPEFFECT_COPY | DROPEFFECT_MOVE | DROPEFFECT_LINK;
        assert_eq!(
            negotiated_drop_effect(MODIFIERKEYS_FLAGS::default(), all),
            DROPEFFECT_MOVE
        );
        assert_eq!(negotiated_drop_effect(MK_CONTROL, all), DROPEFFECT_COPY);
        assert_eq!(negotiated_drop_effect(MK_SHIFT, all), DROPEFFECT_MOVE);
        assert_eq!(
            negotiated_drop_effect(MK_CONTROL | MK_SHIFT, all),
            DROPEFFECT_LINK
        );

        let alt = MODIFIERKEYS_FLAGS(windows::Win32::System::Ole::MK_ALT);
        assert!(drag_modifiers(alt).alt);
        assert_eq!(negotiated_drop_effect(alt, all), DROPEFFECT_LINK);
        assert_eq!(
            negotiated_drop_effect(MK_SHIFT, DROPEFFECT_COPY),
            DROPEFFECT_COPY
        );
    }
}
