...
/// Defers the synchronous OLE modal loop until GPUI's input callback has
/// returned and released its mutable `App` borrow.
pub(crate) const WM_GPUI_START_EXTERNAL_DRAG: u32 = WM_USER + 10;
...
impl WindowsWindowInner {
    pub(crate) fn handle_msg(
        ...
        let handled = match msg {
            ...
            WM_GPUI_START_EXTERNAL_DRAG => self.handle_external_drag(handle),
    ...
    fn handle_external_drag(&self, handle: HWND) -> Option<isize> {
        ...
        // state even when the Shell call failed.
        if let Some(mut input) = self.state.callbacks.input.take() {
            input(PlatformInput::FileDrop(FileDropEvent::Ended));
            self.state.callbacks.input.set(Some(input));
        }
        Some(0)
    }
